...
class CodeMetricsAnalyzer:
    ...
    def get_current_function(self, node):
        """
        Traverse upwards to find the enclosing function for a given node.
        """
        while node:
    ...
    def calculate_halstead_metrics(self):
        ...
        # Get Halstead metrics for the entire code
        halstead_metrics = h_visit(self.code)

        # Access the total metrics
        total_report = halstead_metrics.total

        # Extract total metrics
        unique_operators += total_report.h1  # Unique operators
        unique_operands += total_report.h2  # Unique operands
        total_operators += total_report.N1  # Total operators
        total_operands += total_report.N2  # Total operands
        total_volume += total_report.volume  # Halstead volume
        total_effort += total_report.effort  # Halstead effort

        return unique_operators, unique_operands, total_operators, total_operands, total_volume, total_effort

    def calculate_maintainability_index(self):
        # Pass multi=False to indicate we are analyzing a single file
        mi_score = mi_visit(self.code, multi=False)
        if mi_score is None:  # Handle case where mi_visit might return None
            return 0
        return mi_score  # Return the maintainability index directly
...
# Include the CodeMetricsAnalyzer class and AttributeVisitor class here
...
def main(filepath):
    ...
    results = {
        ...
        "Halstead Metrics": {
            "Unique Operators": operators,
            "Unique Operands": operands,
            "Total Operators": total_ops,
            "Total Operands": total_opnds,
            "Volume": round(volume, 2),
            "Effort": round(effort, 2),
        },
        "Maintainability Index": round(maintainability_index, 2),
